...
def mae_solver(dirname, test_dir,
               convert_consts="no", topk=10):
    """
    Find MAE for all programs from the beam. This should give us an indication
    whether robust estimates will help out?
    """
    # get gold answers
    test_corp = Nlp4plpCorpus(test_dir, convert_consts=convert_consts)
    test_dict = {inst.id: inst.ans for inst in test_corp.insts}
    ...
    # instead of MRR, just calculate MAE for all top-ranked programs
    fs = list(fs)
    for f in tqdm(fs):
        ...
        for n in range(topk):
            ...
            try:
                prob = float(re.findall("^sat.*: (.*)", _out)[0])
                assert 0. <= prob <= 1.
                _y_pred = round(prob, 4)
                _y_true = round(test_dict[f], 4)
                performance = metric([_y_true], [_y_pred])
                preds[test_ids.index(f)].append(performance)
            except IndexError:
                preds[test_ids.index(f)].append(None)
                # preds[test_ids.index(f), n] = 1

    outdir = '/home/fivez/math_problems/'
    outfile = f"{outdir}/train_preds_topk{topk}.json"
...
if __name__ == '__main__':
    ...
    mae_solver(dirname="/home/fivez/math_problems/model_outputs/dev/log_w20191118_151545_853074/",
               test_dir="/home/fivez/math_problems/resources/examples_splits_nums_mapped/dev",
               topk=50)
